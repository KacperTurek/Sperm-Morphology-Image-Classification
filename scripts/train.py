...
def prepare_dataset(data_path, batch, model_path, run_name):
    ...
    test_df.to_csv(f"{model_path}/{run_name}_test_dataset.csv", index=False)
...
def train_model(run_name, num_epochs, patience, params, train_loader, valid_loader, model_path):
    ...
    plt.tight_layout()
    plt.savefig(f"{model_path}/{run_name}_training_results.png")
...
def main(opt):
    ...
    print(f"Results will be saved to {model_path}/{now}_training_results.csv")
    params = {
        "learning_rate": lr,
        "optimizer": optimizer,
        "dropout_fc_rate": dropout_rate,
        "weight_decay": weight_decay,
    }
    train_loader, valid_loader = prepare_dataset(data_path, batch, model_path, now)
    results = train_model(now, epochs, patience, params, train_loader, valid_loader, model_path)
    df = pd.DataFrame([results])
    df.to_csv(f"{model_path}/{now}_training_results.csv", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--optimizer', type=str, default='Adam', help='Available options: Adam dor SGD')
